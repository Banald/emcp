...
A production-grade **Model Context Protocol (MCP) server named Echo** written in TypeScript. It exposes tools to MCP clients (Claude, IDEs, agents) over **Streamable HTTP** transport with custom API key authentication, per-key usage metrics, and a BullMQ-backed background worker system.
