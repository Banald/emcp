...
## Rule 15: Code-execution sandbox (`python-execute` tool)

The `python-execute` MCP tool runs caller-supplied Python in a fresh, isolated container per invocation. Every property in this rule is enforced — `src/shared/sandbox/python-runner.ts` builds the argv with the flags below and `src/tools/python-execute.test.ts` verifies the security probes (network blocked, read-only fs, no host paths) against a real podman container. A reviewer must reject any change that weakens the list.

- **MUST run on a separate container runtime from the eMCP stack itself.** The stack runs against rootless docker; the sandbox runs on **rootless podman**. We deliberately do NOT share the docker daemon with the sandbox: docker-in-docker requires either a privileged container or a bind-mounted socket, both of which violate Rule 14 #1 (no socket sharing). Podman is daemonless and rootless by default, and coexists with rootless docker on the same host.
- **MUST pass user code via stdin.** The runner writes `code` to the spawned process's stdin and never to argv. The Python invocation inside the container is `python3 -u -`, which reads the program from stdin. Verifies via `ps -ef` on the host: only the harmless `python3 -u -` is visible.
- **MUST set `--network=none`.** No outbound traffic. Combined with the read-only rootfs, the only way for sandboxed code to side-channel out is via stdout / stderr, which the runner caps at 1 MiB each.
- **MUST set `--read-only`** on the rootfs and supply only a small `tmpfs` overlay at `/tmp` (`size=64m,mode=1777`). The overlay is per-invocation: it does not persist across runs and is not visible to other invocations. Tested by `python-execute.test.ts` "gives each invocation a fresh /tmp".
- **MUST set `--cap-drop ALL`, `--security-opt no-new-privileges`, and `--user 65534:65534`.** No capabilities, no privilege escalation, runs as `nobody`.
- **MUST set `--memory`, `--memory-swap` (equal), `--pids-limit`, `--cpus`** to bounded values. Defaults: 256 MiB memory, 64 PIDs, 1 CPU. A misbehaving script can't fork-bomb the host or starve other workloads.
- **MUST use a unique `--name` per invocation** (`sandbox-<uuidv4>`). The runner uses this name in the out-of-band `podman kill` it issues on timeout, so kills target the right container even when invocations overlap.
- **MUST enforce wall-clock timeout from the Node parent.** The runner does NOT trust any in-container `timeout` command. It races the child's `close` event against `setTimeout(reject, timeoutMs)`. On timeout it `child.kill('SIGKILL')`s the local podman CLI **and** spawns `podman kill --signal KILL <containerName>` (fire-and-forget). The original `podman run` carries `--rm`, so the container is reaped within `POST_KILL_GRACE_MS` (2 s) of the deadline. Caveat: if the Node parent is SIGKILL'd externally **before** its own timeout fires, the runner never gets to issue either kill — the container persists until its script exits naturally or an operator runs `podman kill`. Every `--network=none` / `--read-only` / `--cap-drop ALL` / memory / PID restriction still applies to the orphan, so it cannot exfiltrate, escalate, or fork-bomb the host. It just consumes its budget until the script ends.
- **MUST cap stdout and stderr** at 1 MiB each in the runner; oversized streams set `truncated: true`.
- **MUST surface every failure as `isError: true`, never throw.** Timeout, syntax error, runtime exception, runtime binary missing, image not built, oversized output — all return a structured tool result with the structured payload populated. Tested by every-failure-path tests in `python-execute.test.ts`.
- **MUST hard-fail the installer** if podman is missing. `scripts/preflight-rootless.sh::check_podman` and `scripts/install.sh::phase_preflight` both refuse to proceed unless `command -v podman` succeeds. The same `EMCP_SKIP_PODMAN_CHECK=1` escape hatch as `EMCP_SKIP_ROOTLESS_CHECK` lets edge-case hosts opt out (the tool will then return `isError` at runtime).
- **MUST log only the size of `code`, never `code` itself.** The handler logs `{ code_bytes: Buffer.byteLength(code, 'utf8') }`. A test walks the logger's call records and asserts the user's code sentinel never appears in any log line.

**Image build + distribution.** `infra/python-sandbox/Dockerfile` is `python:3.12-slim` plus the scientific stack (numpy / pandas / scipy / sympy / matplotlib / scikit-learn) with `USER 65534:65534` and `WORKDIR /tmp`. The release workflow builds, pushes, and **cosign-keyless-signs** the image to `ghcr.io/banald/python-sandbox:vX.Y.Z` alongside the eMCP server image, with the same OIDC identity. Operators verify with the same `cosign verify --certificate-identity-regexp …` incantation in the release notes. `install.sh` `phase_pull_python_sandbox` runs `podman pull` after the stack is healthy; on pull failure (air-gapped host or registry outage) it falls back to a local build via `bash scripts/build-python-sandbox.sh`, so a fresh install always ends with a usable image. The image is **never wired into `compose.yaml`** and never runs at the same trust level as `mcp-server` / `mcp-worker` — the `python-execute` runner is the only caller. If a future change wants to add packages or a different base image, that change must justify the threat-model implication.

**DNS rebinding caveat.** `--network=none` removes all networking, so DNS rebinding doesn't apply. If a future change ever loosens networking (e.g. allows a DNS resolver), `assertPublicHostname` and the egress proxy rules in Rule 13 must be re-evaluated for the sandbox surface.
